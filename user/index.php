...
//Ultimo acceso al campus --Lineas añadidas 
if (!isset($hiddenfields['ultimo_acceso'])) {
    $tablecolumns[] = 'ultimo_acceso';
    $tableheaders[] = get_string('lastaccess')." CV";
}
...
if ($isfrontpage) {
    ...
    $select = "SELECT $mainuserfields, u.lastaccess as ultimo_acceso,COALESCE(ul.timeaccess, 0) AS lastaccess$extrasql";
...
if ($mode === MODE_USERDETAILS) {    // Print simple listing.
    ...
    if ($userlist) {
        ...
        foreach ($userlist as $user) {
            ...
            if (!isset($hiddenfields['lastaccess'])) {
                $data[] = $lastaccess;
            }/* Lineas adicionadas por la DINTEV (que se siente copiar y pegar codigo cada actualización?)*/
             if ($user->ultimo_acceso) {
                   $ultimo_acceso = format_time(time() - $user->ultimo_acceso, $datestring);
                } else {
                  $ultimo_acceso = $strnever;
                }  
                 $data[]= $ultimo_acceso;
